...
namespace MiTutor.Services.TutoringManagement
{
    public class TutorService
    {
        ...
        public async Task<List<TutorContadorProgramasAcademicos>> ListarTutoresEspecialidadConCantidadDeProgramasMod(int SpecialtyId, DateOnly? startDate = null, DateOnly? endDate = null)
        {
            List<TutorContadorProgramasAcademicos> tutores = new List<TutorContadorProgramasAcademicos>();
                ...
                var parameters = new List<SqlParameter>
        {
            ...
            new SqlParameter("@EndDate", endDate.HasValue ? endDate.Value.ToDateTime(TimeOnly.MinValue) : (object)DBNull.Value)
                ...
                DataTable dataTable = await _databaseManager.ExecuteStoredProcedureDataTable("TUTOR_TUTORINGPROGRAMS_LISTARXTUTOR_SELECTXSPECIALTY_MOD", parameters.ToArray());
                if (dataTable != null && dataTable.Rows.Count > 0)
                {
                    foreach (DataRow row in dataTable.Rows)
                    {
            ...
            {
                throw new Exception("ERROR en ListarTutoresEspecialidadConCantidadDeProgramas", ex);
            }
        ...
        public async Task<List<ListarCantidadAppointment>> ListarCantidadAppointmentsEspecialidadMod(int SpecialtyId, DateOnly? startDate = null, DateOnly? endDate = null)
        {
            List<ListarCantidadAppointment> appointments = new List<ListarCantidadAppointment>();
                ...
                var parameters = new List<SqlParameter>
        {
            ...
            new SqlParameter("@EndDate", endDate.HasValue ? endDate.Value.ToDateTime(TimeOnly.MinValue) : (object)DBNull.Value)
                ...
                DataTable dataTable = await _databaseManager.ExecuteStoredProcedureDataTable("TUTOR_APPOINTMENTS_STATUS_COUNT_SELECTXSPECIALTY_MOD", parameters.ToArray());
                if (dataTable != null && dataTable.Rows.Count > 0)
                {
                    foreach (DataRow row in dataTable.Rows)
                    {
