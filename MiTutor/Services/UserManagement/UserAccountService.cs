...
namespace MiTutor.Services.UserManagement
{
    public class UserAccountService
    {
        ...
        public async Task<List<UserAccount>> ListarUsuariosSinRoles()
        {
            List<UserAccount> usuarios = new List<UserAccount>();

            try
            {
                DataTable dataTable = await _databaseManager.ExecuteStoredProcedureDataTable(StoredProcedure.LISTAR_USUARIOS_SIN_ROLES, null);
                if (dataTable != null)
                {
                    foreach (DataRow row in dataTable.Rows)
                    {
